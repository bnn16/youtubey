...
const EditorPosts = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      const link = `http://localhost:8080/rest/requests/edits/posts/editor/${userId}`;
    ...
    <Layout>
      <div className="pb-4">
        ...
        <h2 className="text-xl font-semibold text-gray-700 mt-2">
          All posts where you're the editor.
        </h2>
        ...
        <div className="my-10 grid gap-4 grid-cols-1">
          {posts?.map((post, index) => (
            <div
              key={index}
              className="max-w-xsm bg-white shadow-md rounded-lg"
            >
